...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [pacientesRes, camasRes, medicacionesRes, historialesRes] =
          await Promise.all([
            get("/pacientes"),
            get("/camas"),
            get("/medicamentos"),
        ...
        const pacientes = Array.isArray(pacientesRes)
          ? pacientesRes
          : pacientesRes.data || [];
        const camas = Array.isArray(camasRes) ? camasRes : camasRes.data || [];
        const medicaciones = Array.isArray(medicacionesRes)
          ? medicacionesRes
          : medicacionesRes.data || [];
        const historiales = Array.isArray(historialesRes)
          ? historialesRes
          : historialesRes.data || [];

        const camasOcupadas = camas.filter(
          (c) => c.estado === "ocupada"
        ).length;
    ...
    <div className="col-md-6 col-lg-3 mb-4">
      <Link to={link} className="text-decoration-none">
        <div className="card h-100 border-0 shadow-sm hover-lift overflow-hidden position-relative">
          <div
            className={`position-absolute top-0 end-0 p-3 opacity-10 text-${colorClass}`}
          >
          ...
          <div className="card-body position-relative z-10">
            ...
            <h6 className="text-muted text-uppercase fw-bold small letter-spacing-1 mb-1">
              {title}
            </h6>
          ...
          <div className={`card-footer bg-transparent border-0 pt-0 pb-3`}>
            <span
              className={`badge bg-${colorClass} bg-opacity-10 text-${colorClass} rounded-pill px-3`}
            >
            ...
            <div className="card-body px-4">
              <div className="d-flex align-items-center justify-content-between mb-2">
                ...
                <span className="fw-bold">
                  {stats.camasLibres + stats.camasOcupadas} Camas
                </span>
              </div>
              <div
              ...
              >
                <div
                  ...
                  style={{
                    width: `${
                      (stats.camasOcupadas /
                        (stats.camasLibres + stats.camasOcupadas || 1)) *
                      100
                    }%`,
                  }}
                ></div>
          ...
          <div className="card border-0 shadow-sm h-100 bg-gradient-primary text-white position-relative overflow-hidden">
            ...
            <div className="card-body position-relative z-10 d-flex flex-column justify-content-center p-4">
              ...
              <p className="mb-4 opacity-75">
                Revise los turnos y actividades programadas para hoy.
              </p>
              <Link
                to="/turnos"
                className="btn btn-light text-primary fw-bold border-0 shadow-sm w-100 py-3"
              >
